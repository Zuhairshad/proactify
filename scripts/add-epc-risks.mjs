...
const epcRisks = [
    ...
    },
    {
        ...
        Category: "Engineering",
        Owner: "Lead Engineer",
        DueDate: new Date("2025-01-25").toISOString()
    },
    {
    ...
    },
    {
        ...
        Owner: "Project Manager",
        DueDate: new Date("2025-01-28").toISOString()
    },
    {
        ...
        Category: "Quality",
        Owner: "QA/QC Manager",
        DueDate: new Date("2025-02-12").toISOString()
    },
    {
...
async function addRisks() {
    console.log('Starting to add 20 EPC risks...');

    for (let i = 0; i < epcRisks.length; i++) {
        const risk = epcRisks[i];
        try {
            const response = await fetch('http://localhost:9002/api/risks', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                console.log(`✓ Added risk ${i + 1}/20: ${risk.Title}`);
            } else {
                const error = await response.text();
                console.error(`✗ Failed to add risk ${i + 1}: ${risk.Title}`, error);
            }

            // Small delay to avoid overwhelming the API
            await delay(100);
        } catch (error) {
            console.error(`✗ Error adding risk ${i + 1}:`, error.message);
        }
    }

    console.log('\n✓ Finished adding all risks!');
}

addRisks().catch(console.error);
