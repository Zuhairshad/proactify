...
async function findDocument(id: string): Promise<{ collectionName: string; model: any; data: any } | null> {
    await connectDB();

    // Try to find in risks collection
    const risk = await Risk.findById(id).lean();
    if (risk) {
        return { collectionName: 'risks', model: Risk, data: risk };
    }

    // Try to find in issues collection
    const issue = await Issue.findById(id).lean();
    if (issue) {
        return { collectionName: 'issues', model: Issue, data: issue };
    }
...
export async function updateRiskIssueField(id: string, field: string, value: any) {
    try {
        ...
        await documentInfo.model.findByIdAndUpdate(id, { [field]: value });
...
export async function deleteRiskIssue(id: string) {
    try {
        ...
        await documentInfo.model.findByIdAndDelete(id);
...
export async function changeRiskIssueType(id: string, newType: 'Risk' | 'Issue') {
    try {
        ...
        const { collectionName, data } = documentInfo;
        ...
        delete newData._id; // Remove the old ID
        delete newData.__v; // Remove version key
        ...
        if (newType === 'Risk') {
            ...
            // Get products to map project name to code
            ...
            // Get products to map project code to name
            const products = await Product.find({}).lean();
...
export async function importData(data: any[]) {
    ...
    for (const item of validatedData.data) {
        try {
            ...
            const Model = type === 'Risk' ? Risk : Issue;

            if (id) {
                // Update existing document
                await Model.findByIdAndUpdate(id, itemData);
                updatedCount++;
            } else {
                // Create new document
    ...
    try {
        ...
        return { success: false, message: `Failed to complete import: ${error.message}` };
    }
}
